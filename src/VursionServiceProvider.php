    ...
    public function boot()
    {
        ...
        if ((version_compare(app()->version(), '9.21') >= 0) && class_exists(\Illuminate\Foundation\Console\AboutCommand::class)) {
            \Illuminate\Foundation\Console\AboutCommand::add('Vursion', [
                'Enabled' => function () {
                    return (config('vursion.enabled') ? '<fg=yellow>TRUE</>' : '<fg=yellow>FALSE</>');
                },
                'Key'     => function () {
                    return config('vursion.key');
                },
                'Route'   => function () {
                    return ((version_compare(app()->version(), '5.6.12') >= 0) ? \Illuminate\Support\Facades\URL::signedRoute('vursion') : route('vursion'));
                },
            ]);
        }
    }
